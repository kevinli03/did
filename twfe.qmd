...
The two-way fixed effects (TWFE) estimator is the standard estimator used for classical and generalised DID, when there is non-staggered treatment implementation and absorbing treatment. The model takes the following form.

$$
Y_{it} = \hat\alpha_i + \hat\delta_t + D_{it}\hat\tau_\text{ATT}  + \mathbf X_{it}^\top\hat{\b\beta} + \hat\eps_{it}
$$

The TWFE $\hat\tau_\text{ATT}$ estimate is an unbiased esitmator of the ATT if treatment is non-staggered and absorbing, and the identification assumptions (parallel trends, no anticipation, SUTVA) hold.

::: small
TWFE can be severely biased if there is staggered treatment or non-absorbing treatment.
:::
...
df <- base_stagg
colnames(df) <- c("unit", "time", "cohort", "rel.time", "group", "te", "covar", "outcome")
...
m2 = feols(
  fml   = outcome ~ i(rel.time, group, ref = -1) + covar | unit + time,  # group = treat/never-treat
  data  = df,                                                            # your data
  vcov  = ~ unit                                                         # cluster SE by unit
)
```
...
ggiplot(
  # required options
  object  = m2,                            # input your dynamic model object
