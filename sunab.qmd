...
The **interaction-weighted** (IW) estimator, also called **sunab**, is used for DiD with staggered treatment. The estimator solves the bias of TWFE in staggered settings. The estimator essentially runs smaller DiDs for each different initial treatment period grouping. This is done by interactions in the TWFE model.
...
Then, these individual estimates $\tau_{g, r}$ are aggregated to obtain an overall ATT or dynamic ATTs.
...
m1 = feols(
  fml = outcome ~ sunab(cohort, time) + covar | unit + time,
  data = df,
  vcov = ~ unit
)

# see the variables tab for details on the cohort variable
```
...
ggiplot(
  # required options
  object  = m1,                                  # input your original iw model
  ...
  xlim(-8, 8) # how many period to include.
```
