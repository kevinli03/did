...
We can estimate event-study effects with the `PanelEstimate()` function for post-treatment effects, and the `placebo_test()` function for pre-treatment effects. Since these are separate, we can manually combine the two results from both functions to get event-study estimates.
